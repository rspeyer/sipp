// copied from freeswitch and modified to work in SIPp by ransomr
...
switch_log_printf(int log, int severity, const char * fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	vprintf(fmt, ap);
...
static const struct value_mapping ATTR_TYPES[] = {
	...
	{SWITCH_STUN_ATTR_REALM, "REALM"},
	{SWITCH_STUN_ATTR_NONCE, "NONCE"},
	{SWITCH_STUN_ATTR_XOR_MAPPED_ADDRESS, "XOR_MAPPED_ADDRESS"},
	{SWITCH_STUN_ATTR_PRIORITY, "PRIORITY"},
	{SWITCH_STUN_ATTR_USE_CANDIDATE, "USE_CANDIDATE"},
	{SWITCH_STUN_ATTR_OPTIONS, "OPTIONS"},
	{SWITCH_STUN_ATTR_SOFTWARE, "SOFTWARE"},
	{SWITCH_STUN_ATTR_ALTERNATE_SERVER2, "ALTERNATE_SERVER2"},
	{SWITCH_STUN_ATTR_FINGERPRINT, "FINGERPRINT"},
	{SWITCH_STUN_ATTR_ICE_CONTROLLED, "ICE_CONTROLLED"},
	{SWITCH_STUN_ATTR_ICE_CONTROLLING, "ICE_CONTROLLING"},
	{SWITCH_STUN_ATTR_RETRANSMIT_COUNT, "RETRANSMIT_COUNT"},
	{0, 0}
};
...
SWITCH_DECLARE(uint32_t) switch_stun_compute_crc32(const void* data, size_t length)
{
...
// IMPORTANT: This method assumes that the length and type values of the attribute have ALREADY been converted from network order to host (ntohs)
SWITCH_DECLARE(uint8_t) switch_stun_validate_packet_attribute(switch_stun_packet_attribute_t *attribute, void *end)
{
	if (!attribute) {
#ifdef DEBUG_STUN_PARSE
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "STUN: Invalid attribute, breaking!\n");
#endif
		attribute = NULL;
		return 0;
	}

	// With the introduction of USE_CANDIDATE in RFC 5245 it is now legal to have a zero-length attribute
	if (!attribute->type 
		|| ((attribute->type != SWITCH_STUN_ATTR_USE_CANDIDATE) && !attribute->length)
		|| ((void *) attribute > end)) {
		attribute = NULL;
		return 0;
	}

	if ((void *)(attribute + switch_stun_attribute_padded_length(attribute)) > end) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "STUN: Invalid attribute length (runs off end) [ATTRIBUTE=%p] [TYPE=%u] [LENGTH=%u] [PADDED_LENGTH=%u] [END=%p], breaking!\n", (void *)attribute, attribute->type, attribute->length, switch_stun_attribute_padded_length(attribute), end);
		attribute = NULL;
		return 0;
	}
...
{
	uint16_t length = (uint16_t)attribute->length;
	if ((length % 4) != 0) {
		length += (4 - (length % 4));
	}
	return length;
...
{
	// Advance pointer
	*attribute = (switch_stun_packet_attribute_t *) ((*attribute)->value + switch_stun_attribute_padded_length(*attribute));

	return (*attribute) ? SWITCH_STATUS_SUCCESS : SWITCH_STATUS_FALSE;
}
...
SWITCH_DECLARE(switch_stun_packet_t *) switch_stun_packet_parse(uint8_t *buf, uint32_t len, switch_stun_packet_data_t *data, switch_memory_pool_t *pool)
{
	...
	if (len < SWITCH_STUN_PACKET_MIN_LEN) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "STUN: Invalid length (too short) %d!\n", len);
		return NULL;
	}

    memset(data, 0, sizeof(switch_stun_packet_data_t));
	...
	default:
		/* Invalid value */
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "STUN: Invalid packet type: %u\n", packet->header.type);
		return NULL;
	}
	 ...
	 */
	if ((len - (packet->header.length + 20)) <= 0) {
		/* Invalid */
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "STUN: Invalid length!\n");
		return NULL;
	}
	...
	if (packet->header.length == 0) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "STUN: No length!\n");
		return NULL;
	}
	...
	do {
		...
		// Only validate packet once we have converted to host format
		if (!switch_stun_validate_packet_attribute(attr, end_buf)) {
			break;
		}

#ifdef DEBUG_STUN_PARSE
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "STUN: Parsing attribute (%p) [TYPE=%u %s] [LENGTH=%u] [PADDED_LENGTH=%u]\n", (void *)attr, attr->type, switch_stun_value_to_name(SWITCH_STUN_TYPE_ATTRIBUTE, attr->type), attr->length, switch_stun_attribute_padded_length(attr));
#endif
		...
		case SWITCH_STUN_ATTR_SOURCE_ADDRESS2:
			{
				...
				default:		/* Invalid */
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "STUN: Invalid IP family:%d!\n", ip->family);
					return NULL;
				}
				...
				if (attr->length != addr_length + 4) {
					/* Invalid */
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "STUN: Invalid IP address length:%u!\n", attr->length);
					return NULL;
				}
		...
		case SWITCH_STUN_ATTR_OPTIONS:
			...
			break;
		case SWITCH_STUN_ATTR_PRIORITY:	/* UInt32 */
			{
				uint32_t *val = (uint32_t *) attr->value;
		...
		case SWITCH_STUN_ATTR_ICE_CONTROLLED:	/* UInt64 */
		case SWITCH_STUN_ATTR_ICE_CONTROLLING:
			{
				uint64_t *val = (uint64_t *) attr->value;

				if (attr->length != sizeof(uint64_t)) {
					/* Invalid */
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "STUN: Invalid ICE_CONTROLLING/CONTROLLED length:%u!\n", attr->length);
					return NULL;
				}

				*val = ntohl(*val);	/* should we do this here? */

				if (attr->type == SWITCH_STUN_ATTR_ICE_CONTROLLING) {
					data->controlling = 1;
				}
			}
			break;

		case SWITCH_STUN_ATTR_USE_CANDIDATE: /* no length */
			{
				if (attr->length != 0) {
					/* Invalid */
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "STUN: Invalid USE_CANDIDATE length:%u!\n", attr->length);
					return NULL;
				}
				data->use_candidate = 1;
			}
			break;
		case SWITCH_STUN_ATTR_USERNAME:	/* ByteString, multiple of 4 bytes */
			{
				uint16_t len = switch_stun_attribute_padded_length(attr);
				char username[SWITCH_STUN_USERNAME_PADDED_LENGTH] = { 0 };
				if (len != SWITCH_STUN_USERNAME_PADDED_LENGTH) {
					/* Invalid */
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "STUN: Invalid USERNAME length:%u!\n", attr->length);
					return NULL;
				}
				
				memcpy(username, attr->value, len);
				data->username = switch_core_strdup(pool, username);
			}
			break;
		case SWITCH_STUN_ATTR_PASSWORD:	/* ByteString, multiple of 4 bytes */
			{
				if ((attr->length - 1) % 4 != 0) {
					/* Invalid */
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "STUN: Invalid PASSWORD length:%u!\n", attr->length);
					return NULL;
				}
			}
			break;
		...
		case SWITCH_STUN_ATTR_MESSAGE_INTEGRITY:	/* ByteString, 20 bytes */
			// TODO rspeyer VALIDATE
			if (attr->length != 20) {
				/* Invalid */
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "STUN: Invalid MESSAGE_INTEGRITY length:%u!\n", attr->length);
				return NULL;
			}
			break;
		...
		case SWITCH_STUN_ATTR_MAGIC_COOKIE:	/* ByteString, 4 bytes */
			if (attr->length != 4) {
				/* Invalid */
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "STUN: Invalid MAGIC_COOKIE length:%u!\n", attr->length);
				return NULL;
			}
			break;
		...
		case SWITCH_STUN_ATTR_UNKNOWN_ATTRIBUTES:	/* UInt16List (= multiple of 2 bytes) */
			if (attr->length % 2 != 0) {
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "STUN: Invalid UNKNOWN_ATTRIBUTES length:%u!\n", attr->length);
				return NULL;
			}
			break;
		...
		case SWITCH_STUN_ATTR_FINGERPRINT:
			{
				...
				if (attr->length != sizeof(uint32_t)) {
					/* Invalid */
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "STUN: Invalid FINGERPRINT length:%u!\n", attr->length);
					return NULL;
				}

				*val = ntohl(*val);	/* should we do this here? */

				// According to RFC5389, Section 15.5 the FINGERPRINT MUST be the last STUN attribute 
				// So, mark this as the end so we ignore anything after it
				reached_end = 1;
			}
			break;
		...
		default:
#ifdef DEBUG_STUN_PARSE
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "STUN: Unhandled attribute type:%u!\n", attr->type);
#endif
			/* Mandatory attribute range? => invalid */
			if ((attr->type <= 0x7FFF)) {
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "STUN: Invalid attribute type:%u!\n", attr->type);
				return NULL;
			}
			break;
		}
...
{
	switch_stun_packet_attribute_t *attribute;
	uint64_t tiebreaker = 1;

	attribute = (switch_stun_packet_attribute_t *) ((uint8_t *) & packet->first_attribute + ntohs(packet->header.length));
	attribute->type = htons(SWITCH_STUN_ATTR_ICE_CONTROLLED);
	attribute->length = htons(sizeof(tiebreaker));
	memcpy(attribute->value, &tiebreaker, sizeof(tiebreaker));
...
SWITCH_DECLARE(uint8_t) switch_stun_packet_attribute_add_ice_controlling(switch_stun_packet_t *packet)
{
	...
	attribute = (switch_stun_packet_attribute_t *) ((uint8_t *) & packet->first_attribute + ntohs(packet->header.length));
	attribute->type = htons(SWITCH_STUN_ATTR_ICE_CONTROLLING);
	attribute->length = htons(sizeof(tiebreaker));
	memcpy(attribute->value, &tiebreaker, sizeof(tiebreaker));
...
SWITCH_DECLARE(uint8_t) switch_stun_packet_attribute_add_use_candidate(switch_stun_packet_t *packet)
{
	...
	attribute = (switch_stun_packet_attribute_t *) ((uint8_t *) & packet->first_attribute + ntohs(packet->header.length));
	attribute->type = htons(SWITCH_STUN_ATTR_USE_CANDIDATE);
...
SWITCH_DECLARE(uint8_t) switch_stun_packet_add_priority(switch_stun_packet_t *packet, uint32_t priority)
{
	switch_stun_packet_attribute_t *attribute;

	attribute = (switch_stun_packet_attribute_t *) ((uint8_t *) & packet->first_attribute + ntohs(packet->header.length));
	attribute->type = htons(SWITCH_STUN_ATTR_PRIORITY);
	attribute->length = htons(sizeof(priority));
	memcpy(attribute->value, &priority, sizeof(priority));
...
SWITCH_DECLARE(uint8_t) switch_stun_packet_attribute_add_fingerprint(switch_stun_packet_t *packet)
{
	...
	crc = htonl(switch_stun_compute_crc32(packet, xlen) ^ STUN_FINGERPRINT_XOR);
...
{
	switch_stun_packet_attribute_t *attribute;
	uint8_t class = (uint8_t)(error / 100);
	uint8_t number = (uint8_t)(error % 100);
	uint32_t padded_code = htonl((class << 8 | number));

	attribute = (switch_stun_packet_attribute_t *) ((uint8_t *) & packet->first_attribute + ntohs(packet->header.length));
	attribute->type = htons(SWITCH_STUN_ATTR_ERROR_CODE);
	attribute->length = htons(sizeof(padded_code));
	memcpy(attribute->value, &padded_code, sizeof(padded_code));
												   ...
												   switch_port_t *port, char *stunip, switch_port_t stunport, char **err, switch_memory_pool_t *pool)
{
	...
	void *end_buf;
	switch_stun_packet_t *packet;
	switch_stun_packet_data_t *stun_data = NULL;
	switch_stun_packet_attribute_t *attr;
	...
	*port = 0;
	...
	packet = switch_stun_packet_parse(start, size, stun_data, pool);
	if (!packet) {
